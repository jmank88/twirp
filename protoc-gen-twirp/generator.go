...
func (t *twirp) generateImports(file *descriptor.FileDescriptorProto) {
	...
	t.P(`import `, t.pkgs["ubjson"], ` "github.com/jmank88/ubjson"`)
	t.P()
...
func (t *twirp) generateUtils() {
	...
	t.generateDoRequest("Protobuf", "application/protobuf", func() {
	...
	t.generateDoRequest("JSON", "application/json", func() {
	...
	t.generateDoRequest("UBJSON", "application/ubjson", func() {
		t.P(`  reqBody := `, t.pkgs["bytes"], `.NewBuffer(nil)`)
		t.P(`  encoder := `, t.pkgs["ubjson"], `.NewEncoder(reqBody)`)
		t.P(`  if err = encoder.Encode(in); err != nil {`)
		t.P(`    return clientError("failed to marshal json request", err)`)
		t.P(`  }`)
	}, func() {
		t.P(`  decoder := `, t.pkgs["ubjson"], `.NewDecoder(resp.Body)`)
		t.P(`  if err = decoder.Decode(out); err != nil {`)
		t.P(`    return clientError("failed to unmarshal ubjson response", err)`)
		t.P(`  }`)
		t.P(`  if err = ctx.Err(); err != nil {`)
		t.P(`    return clientError("aborted because context was done", err)`)
		t.P(`  }`)
...
func (t *twirp) generateDoRequest(kind, content string, request, response func()) {
	t.P(`// do`, kind, `Request is common code to make a request to the remote twirp service.`)
	t.P(`func do`, kind, `Request(ctx `, t.pkgs["context"], `.Context, client HTTPClient, url string, in, out `, t.pkgs["proto"], `.Message) error {`)
	t.P(`  var err error`)
	request()
	t.P(`  if err = ctx.Err(); err != nil {`)
	t.P(`    return clientError("aborted because context was done", err)`)
...
func (t *twirp) generateService(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto, index int) {
	...
	t.sectionComment(servName + ` UBJSON Client`)
	t.generateClient("UBJSON", file, service)
...
func (t *twirp) generateServer(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) {
	...
	// Methods.
	for _, method := range service.Method {
		t.generateServerMethods(service, method)
	}
...
func (t *twirp) generateServerMethods(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto) {
	...
	t.P(`  case "application/ubjson":`)
	t.P(`    s.serve`, methName, `UBJSON(ctx, resp, req)`)
	t.P(`  default:`)
	...
	t.P(`}`)
	t.P()
	t.generateServerMethod(service, method, "JSON", "application/json", func() {
	...
	t.generateServerMethod(service, method, "Protobuf", "application/protobuf", func() {
	...
	t.generateServerMethod(service, method, "UBJSON", "application/ubjson", func() {
		t.P(`  reqContent := new(`, t.goTypeName(method.GetInputType()), `)`)
		t.P(`  decoder := `, t.pkgs["ubjson"], `.NewDecoder(req.Body)`)
		t.P(`  if err = decoder.Decode(reqContent); err != nil {`)
		t.P(`    err = wrapErr(err, "failed to parse request ubjson")`)
		t.P(`    s.writeError(ctx, resp, `, t.pkgs["twirp"], `.InternalErrorWith(err))`)
		t.P(`    return`)
		t.P(`  }`)
	}, func() {
		t.P(`  respBytes, err := `, t.pkgs["ubjson"], `.Marshal(respContent)`)
		t.P(`  if err != nil {`)
		t.P(`    err = wrapErr(err, "failed to marshal ubjson response")`)
		t.P(`    s.writeError(ctx, resp, `, t.pkgs["twirp"], `.InternalErrorWith(err))`)
		t.P(`    return`)
		t.P(`  }`)
	})
...
func (t *twirp) generateServerMethod(service *descriptor.ServiceDescriptorProto, method *descriptor.MethodDescriptorProto, kind, content string, input, output func()) {
	servStruct := serviceStruct(service)
	methName := stringutils.CamelCase(method.GetName())
	t.P(`func (s *`, servStruct, `) serve`, methName, kind, `(ctx `, t.pkgs["context"], `.Context, resp `, t.pkgs["http"], `.ResponseWriter, req *`, t.pkgs["http"], `.Request) {`)
	t.P(`  var err error`)
