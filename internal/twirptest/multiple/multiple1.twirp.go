...
type svc1UBJSONClient struct {
	client HTTPClient
	urls   [1]string
}

// NewSvc1UBJSONClient creates a UBJSON client that implements the Svc1 interface.
// It communicates using UBJSON and can be configured with a custom HTTPClient.
func NewSvc1UBJSONClient(addr string, client HTTPClient) Svc1 {
	prefix := urlBase(addr) + Svc1PathPrefix
	urls := [1]string{
		prefix + "Send",
	}
	if httpClient, ok := client.(*http.Client); ok {
		return &svc1UBJSONClient{
			client: withoutRedirects(httpClient),
			urls:   urls,
		}
	}
...
func (c *svc1UBJSONClient) Send(ctx context.Context, in *Msg1) (*Msg1, error) {
	out := new(Msg1)
...
func (s *svc1Server) serveSendUBJSON(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	var err error
	ctx = ctxsetters.WithMethodName(ctx, "Send")
	ctx, err = callRequestRouted(ctx, s.hooks)
	if err != nil {
